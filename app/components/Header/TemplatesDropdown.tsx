...
export default function TemplatesDropdown() {
  ...
  return (
    <div className="TemplatesDropdown">
      <HeaderMenuDataWrap
        fallback={
          <div className="py-2 h-10 sm:h-12 flex items-center rounded-md text-sm md:text-base font-medium text-slate-700 dark:text-slate-300 hover:text-slate-900 focus:outline-none focus-visible:ring-0">
            <span>Await</span>
            ...
            {({open, close}) => (
              <>
                <PopoverButton
                  className={`
                  ${open ? '' : 'text-slate-700'}
                  group py-2 h-10 sm:h-12 dark:text-slate-300 hover:text-slate-900 flex items-center rounded-md text-base text-nowrap focus:outline-none focus-visible:ring-0`}
                >
                  <span>{'Shop Now' || headerMenu?.title || 'Shops'}</span>
                  <ChevronDownIcon
